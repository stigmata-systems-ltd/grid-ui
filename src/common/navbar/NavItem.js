...
class NavItem extends Component {
    ...
    renderSubNavs = (subNavs, id) => {
        return (subNavs.map((subNav) =>
            <div class={`collapse ${(this.state.isSubNavOpen && this.state.activeId === id) && "show"}`}>
    ...
    render() {
        return (
            ...
            <li class={`nav-item ${this.state.activeId === navItem.id && "active"}`}>
                <button
                    key={navItem.id}
                    class={`nav-link ${(this.state.isSubNavOpen &&  this.state.activeId === navItem.id) && "show"}`}
